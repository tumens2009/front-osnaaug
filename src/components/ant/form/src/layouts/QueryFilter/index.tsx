...
import { useIntl } from '@/components/ant/provider/src';// '@/components/ant/provider/src';
import { isBrowser, useMountMergeState } from '@/components/ant/utils/src';
...
const QueryFilterContent: React.FC<{
  ...
  const intl = useIntl();
  const resetText = props.resetText || intl.getMessage('tableForm.reset', 'Reset');
  const searchText = props.searchText || intl.getMessage('tableForm.search', 'Search');
