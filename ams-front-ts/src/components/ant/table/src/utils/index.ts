...
import type { UseEditableUtilType } from '@/components/ant/utils/src';
import { arrayMoveImmutable } from '@/components/ant/utils/src';
...
export function mergePagination<T>(
  ...
  return {
    showTotal: (all, range) =>
      `${intl.getMessage('pagination.total.range', 'First')} ${range[0]}-${
        range[1]
      } ${intl.getMessage('pagination.total.total', 'Article/Total')} ${all} ${intl.getMessage(
